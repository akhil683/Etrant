...
export interface IWikipediaRepository {
  ...
  getArticlesByInterests(interests: InterestCategory[], count: number): Promise<Article[]>
}

export class WikipediaRepository extends BaseRepository implements IWikipediaRepository {
  ...
  async getRandomArticle(category: string): Promise<Article> {
    return this.handleRequest(async () => {
      try {
        ...
        if (!response.ok) {
          throw new WikipediaAPIError(`Wikipedia API returned ${response.status}`)
        }
        ...
        this.logger.warn("Wikipedia API failed, using fallback", { category, error })
        return this.getFallbackArticle(category)
      }
    }, "Failed to fetch Wikipedia article")
  }

  async getArticlesByInterests(interests: InterestCategory[], count: number): Promise<Article[]> {
    ...
    for (let i = 0; i < count; i++) {
      ...
      try {
        ...
        articles.push({
          ...article,
          topic: randomInterest.charAt(0).toUpperCase() + randomInterest.slice(1),
        })
      } catch (error) {
        this.logger.error("Error fetching article", { interest: randomInterest, error })
        continue
      }
    }
